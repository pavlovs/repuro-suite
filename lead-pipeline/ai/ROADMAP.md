...
| **Batch funnel showed ingest stages, not the batch process** — Filtered/Scraped/Classified say nothing for a manually assembled batch; the two real steps (OUT in the batch Excel / Duplikat / Excluded, then the manual size/ownership check) and their reasons were only in Drop-Off | Medium | Fixed 2026-09-21 — Auswertung › Funnel with a batch selected = `_computeDropoffBatch` (Zugewiesen → Vorprüfung → Manuelle Prüfung → Freigabe → Versendet) with drop reasons per step from `reclassify_reason`/prio; works for every batch; Drop-Off split the same way |
| **Prod OOM-killed on page loads** (512 MB machine shared by all modules; 5 kills 2026-09-22, browser saves in the gaps lost as 502): every page load built a ~9 MB JSON of all records, 81% of it `scraped_text` the UI never shows beyond 180 chars, half of the rest repeated key names + null fields | High | Fixed 2026-09-22: `scraped_text` cut to `settings.DASHBOARD_SCRAPED_EXCERPT_CHARS` in `_load_data`, null fields dropped in `_payload_json` (page data + `/api/data`); payload 9,1 → 2,8 MB (−70%), page 9,6 → 3,3 MB; hot-patched on prod (settings.py + dashboard.py + restart), on dev for the next deploy. Full text stays in `pipeline.db` for the pipeline stages |
| **Filter-Grund / Ownership not editable in the Lead-Liste** (readonly columns + not in `_WRITEBACK_FIELDS`; the batch "Prio 2" action's `filter_reason` patch was silently dropped by the server for the same reason) | Medium | Fixed 2026-09-23: text areas with blur-save, `filter_reason` + `ownership_reason` whitelisted; hot-patched on prod (template + dashboard.py + restart). Note `filter_pass` is still not whitelisted, so that batch action's `filter_pass: 0` is still dropped |
...
| **Lead-Liste prio dropdown lacks `Duplicate`**: Duplicate records render as "Prio 1"; a save would overwrite | Medium | Fixed 2026-09-22: `Duplicate` added to the row prio select (TICKETS T008); on dev, ships with next deploy |
| **Lead-Liste had only a "Prio 2 anzeigen" toggle**: no way to slice the list by prio, MA, region or klass | Medium | Fixed 2026-09-22: multi-select filter toolbar (queue-filter popover component): Batch / Prio (incl. "Prio 2 (alle)", "(leer)") / Klass / Region with facet counts (a list counts what the other filters + search + MA leave over; 0-count options hidden unless ticked) + search for long lists, MA von/bis, "Filter zurücksetzen"; OR within a field, AND across fields; persists per browser (`allex_leads_filters`, arrays); new dimension = one line in `LEADS_FILTER_DEFS`; columns Rechtsform + Reklassifiz. dropped, Name/Prio cells never clipped, "↗" website shortcut after the name; hot-patched on prod 2026-09-22 (template + process restart), on dev for the next deploy |
