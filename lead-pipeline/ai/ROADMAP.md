...
| **Lead-Liste had only a "Prio 2 anzeigen" toggle**: no way to slice the list by prio, MA, region or klass | Medium | Fixed 2026-09-22: filter toolbar: Batch / Prio (incl. "Prio 2 (alle)", "(leer)") / Klass / Region selects built from the data + MA von/bis + "Filter zurücksetzen"; selection persists per browser (`allex_leads_filters`); new dimension = one line in `LEADS_FILTER_DEFS`; on dev, ships with next deploy |
