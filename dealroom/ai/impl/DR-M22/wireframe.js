/**
 * DR-M22 — Deal History / Current Status Wireframe
 * Section 3 of the DEALROOM dashboard.
 * [Wireframe — mock data]
 *
 * Exported function: renderDealHistory()
 * Returns a self-contained HTML string. No external libraries.
 */
...
// ---------------------------------------------------------------------------
// Sub-renderer: Key Dates
// ---------------------------------------------------------------------------
function renderKeyDates(deal) {
  const lastContactDays = daysSince(deal.last_contact_at);
  const ndaDays = daysSince("2025-12-04");
  ...
  const rows = [
    ...
    ["Last contact",     fmtDate(deal.last_contact_at), `<span class="dh-badge-warn">${lastContactDays}d ago</span>`],
...
// ---------------------------------------------------------------------------
// Sub-renderer: Recent Activity Feed
// ---------------------------------------------------------------------------
  ...
  const rowsHtml = notes.map(n => {
    ...
    return `
      <tr class="dh-feed-row">
        <td class="dh-feed-date">${fmtDate(n.date)}</td>
...
// ---------------------------------------------------------------------------
// Sub-renderer: Open Actions / Blockers
// ---------------------------------------------------------------------------
...
function renderDealHistory() {
  // ---------------------------------------------------------------------------
  // Mock data (matches DATA.deal / DATA.notes shape expected by production)
  // ---------------------------------------------------------------------------
  const MOCK = {
    deal: {
      name: "HWV GmbH",
  ...
  // Read from global DATA if available; fall back to mock data (wireframe mode)
  ...
  const css = `
    <style>
      ...
      .dh-tl-line {
        ...
        height: 2px;
        min-width: 20px;
        margin-top: -26px; /* align with dot center */
...
// ---------------------------------------------------------------------------
// Standalone preview — open this file in a browser to see the wireframe
// ---------------------------------------------------------------------------
if (typeof document !== "undefined") {
  document.addEventListener("DOMContentLoaded", () => {
    document.body.style.margin = "24px";
    document.body.style.background = "#e2e8f0";
    document.body.innerHTML = renderDealHistory();
  });
}

// Node.js / module export
if (typeof module !== "undefined" && module.exports) {
  module.exports = { renderDealHistory };
}
