...
- [x] **Lead-Liste columns** (RD, Roman 2026-09-22): Rechtsform (empty/junk for all but 66 rows) and Reklassifiz. (duplicates Filter-Grund) removed; Name input sized to its text (column grows with the longest name, min 300px) and the row Prio select 130px min so "Prio 2 (other)" is never cut. Verified 2026-09-22 headless (no clipped name on a 100-row page, no horizontal page scroll).
- [x] **Lead-Liste website shortcut** (RD, Roman 2026-09-22): "↗" after the name opens https://<domain> in a new tab (Roman: a shortcut, not a column). Verified 2026-09-22 headless (link per row, new tab, no clipping).
